...
class PatchCore(torch.nn.Module):
    def __init__(self, device, save_patch_scores=False,save_residuals=False,ref_num_patches=None,add_pos_embed=True,
                 context=True,context_weight=1,visible=False,pos_weight=1,mode='cpu',pca_com=0.95):
    ...
    def _fill_memory_bank(self, input_data,bankpath):
        ...
        if bankpath is not  None and os.path.exists(bankpath):
                ...
                for batch in data_iterator:
                    if isinstance(batch, dict):
                        ...
                        if self.context:
                            ...
                            context = None
                    images.append(image)
                    features.append(_image_to_features(image,context))
            ...
            if features.shape[0] > 1000000:
                subset_indices = np.random.choice(
                    features.shape[0], 1000000, replace=False
                )
            ...
            images = torch.nn.functional.unfold(torch.tensor(images), 8,
                                                stride=8).transpose(1, 2)
        ...
        if self.context:
            ...
            contexts = contexts.permute(0,3,1,2)
            contexts = F.interpolate(
                contexts,
                size=(self.ref_num_patches[0], self.ref_num_patches[1]),
                mode="bilinear",
                align_corners=False,
            )
            self.contexts = contexts.permute(0,2,3,1).reshape(-1,contexts.shape[1]).numpy()[sample_indices]
        # if self.visible:



    def _predict(self, images,ids, context=None,nn_num=1,p=None ):
        ...
        with torch.no_grad():
            ...
            patch_scores,residuals,query_nns = self.anomaly_scorer.predict(ref_features,ref_pca_features,
                features,pca_features,nn_num,p)
            ...
            # else:
            #     residuals = residuals.reshape(batchsize,scales[0],scales[1],-1).transpose((0,3,1,2))
