...
for single_dataset in ['KolektorSDD2']:#sorted(os.listdir(root_dir)):
    ...
    for dset in ['train','test']:
        for category in ['ok','ng','false_ng']:
            ...
            if dset == 'train' and category == 'false_ng':
                path_list = path_info[single_dataset][dset]['ok']
                alpha_list = path_info[single_dataset][dset][f'ok_binary']
                ...
                path_list = path_info[single_dataset][dset]['ng']
                alpha_list = path_info[single_dataset][dset][f'ng_binary']
            else:
            ...
            for i, image_path in tqdm(enumerate(path_list)):
                image_name = image_path.split('/')[-2] + '_' +os.path.basename(image_path).split('.', 1)[0]
                # ref_path = ref_list[i]
                ...
                if dset == 'train' and category == 'false_ng':
                        while True:
                            ...
                            if  np.min(np.array(stats)[:,-1]) > 250:#input_size[0]*input_size[1]*0.005:
