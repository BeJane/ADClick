...
from dataset.config import exp_path, mvtec_root_dir, out_mvtec_root_dir, categories, all_sets
...
ann_dir = '/media/szcycy/E/SimpleClickRes/model_zero_conv/log/mvtec_zero_conv_plainvit_base448_finetune/vi_results/mvtec'

with open(exp_path,'r',encoding='utf-8') as f:
    path_info = json.load(f)

for single_dataset in sorted(all_sets):
# for single_dataset in ['BS','KolektorSDD','KolektorSDD2','magnetic_tile','cam0','cam2','cam3','LAO','YITENG']:
    ...
    for i, image_path in tqdm(enumerate(path_list)):
        ...
        ori_pha_image = np.load(os.path.join(ann_dir,single_dataset,dset,category,f'{dset}_{category}_{i}_{ori_image_name}_c5.npy'))
        ori_pha_image[ori_pha_image > 0.49] = 255
        ori_pha_image[ori_pha_image < 255] = 0
        ori_pha_image = cv2.resize(ori_pha_image,(input_size[0],input_size[-1]),interpolation=cv2.INTER_NEAREST)
