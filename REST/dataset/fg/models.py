...
class BaseModel(nn.Module):
    def __init__(self, output_dim: int = 384, backbone_name: str = 'densenet201', pretrained: bool = True, freeze_pre_bn: bool = True, layers: List[str] = ['features.denseblock1', 'features.denseblock2']):
        """
        Args:
        ...
        self.layers = layers
        self.freeze_modules = []
        self.backbone = self.load_backbone(backbone_name, pretrained).eval()
...
class ResnetPM(BaseModel):
    def __init__(self, output_dim: int = 384, backbone_name: str = 'resnet18', pretrained: bool = True, freeze_pre_bn: bool = True, layers: List[str] = ['layer1', 'layer2']):
        super().__init__(output_dim, backbone_name, pretrained, freeze_pre_bn, layers)
...
class EfficientnetPM(BaseModel):
    def __init__(self, output_dim: int = 384, backbone_name: str = 'efficientnet_b0', pretrained: bool = True, freeze_pre_bn: bool = True, layers: List[str] = ['features.2', 'features.3']):
        super().__init__(output_dim, backbone_name, pretrained, freeze_pre_bn, layers)
...
class MobilenetPM(BaseModel):
    def __init__(self, output_dim: int = 384, backbone_name: str = 'mobilenet_v3_small', pretrained: bool = True, freeze_pre_bn: bool = True, layers: List[str] = ['features.2.block.0', 'features.4.block.0']):
        super().__init__(output_dim, backbone_name, pretrained, freeze_pre_bn, layers)
...
class SuperPointNetPM(BaseModel):
    def __init__(self, output_dim: int = 384, backbone_name: str = 'superpoint_v1', pretrained: bool = True, freeze_pre_bn: bool = True, layers: List[str] = ['conv3b', 'conv4b']):
        super().__init__(output_dim, backbone_name, pretrained, freeze_pre_bn, layers)
    
    def load_backbone(self, backbone_name, pretrained) -> SuperPointNet:
        backbone = SuperPointNet()
        if pretrained:
            backbone = load_ckpt(backbone, torch.load(f'./pretrain_weights/{backbone_name}.pth'))
        return backbone
