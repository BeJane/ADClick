...
our_dir = '/media/wjq/F/Projects/vit-defect-detection/results/weight0.1_aug532_residual_slide32_8_swin_ws8_head32_depths4_alpha0.25_332_exp1'

draem_dir =  '/media/wjq/F/Dataset/MVTEC/DevNet_preds'
...
# for category in ['bottle']:
    for anomaly in os.listdir(os.path.join(mvtec_dir ,category,'ground_truth')):
        ...
        for p in paths:
            ...
            if not os.path.exists(p.replace(our_dir,draem_dir)):continue
            ...
            axs[3].imshow(cv2.resize(img,(our_pred.shape[0],our_pred.shape[1])))
            axs[3].imshow(our_pred, alpha=0.4,cmap='jet')

            # plt.tight_layout()
            # plt.show()
            save_path = os.path.join(our_dir,category,f'devnet_{anomaly}_{bsn}.png')
