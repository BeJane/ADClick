...
from REST.util.mvtec_shape_surface import MVTecSurfaceShapeTextDataset, surface, shape
from REST.util.util import fix_seed

text_dir = '/home/szcyxy/SSD/ADclick/data/defect_1024/mvtec_text'
...
dataset = MVTecSurfaceShapeTextDataset(text_dir)
...
for path in pathlist:
    ...
    with open(path,'rb') as f:
        ...
        for k, v in prompt_info.items():
            ...
            if (anomaly not in surface) and (anomaly not in shape):
                print(anomaly)
                continue
    ...
    with open(path.replace("anomaly","surface_shape_classifier"), 'wb') as f:
