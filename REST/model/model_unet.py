...
from model.util import random_masking, prepocess_residual
...
class DiscriminativeSubNetwork(nn.Module):
    def __init__(self,in_channels=3, out_channels=3, base_channels=64, out_features=False,residual_method='square'):
    ...
    def forward(self, x, mask_ratio=None):
        ...
        # print(output_segment.shape)
        # for i in range(output_segment.shape[0]):
        # plt.imshow(output_segment[-1,1].detach().cpu())
        # plt.show()
        output_segment = output_segment.permute(0,2,3,1).flatten(1,2)
        # print(output_segment.shape)


        if mask_ratio is not None:
            return output_segment,mask_pos
