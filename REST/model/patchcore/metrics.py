...
def compute_pro_torch(gts, preds, integration_limit=0.3):
    ...
    components = connected_components(gts.unsqueeze(1).type(torch.float),
                                      num_iterations=1000)  # kornia expects N1HW format, kornia expects FloatTensor
    ...
    for label in labels:
        ...
        mask = cca == label
        _fpr, _tpr = binary_roc(preds=preds.flatten()[background | mask], target=mask[background | mask],
