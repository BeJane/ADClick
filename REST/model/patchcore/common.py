...
class NearestNeighbourScorer(object):
    ...
    def predict(
        self, query_features: List[np.ndarray],nn_num,p,method='square'
    ) -> Union[np.ndarray, np.ndarray, np.ndarray]:
        ...
        query_distances, query_nns = self.nn_method.run(nn_num,query_features)
        # print(nn_num,query_distances[0])
        if nn_num > 1:
            ...
            indexs = np.random.choice(np.arange(query_distances.shape[1]),size=query_distances.shape[0], p=p)
            query_single_nns = query_nns[(np.arange(query_distances.shape[0]),indexs)]

            # print(query_single_nns,np.mean((query_distances[:,1] +(1e-10))/( query_distances[:,0]+(1e-10))),
            #       np.min((query_distances[:,1] +(1e-10))/( query_distances[:,0]+(1e-10))))

            # self.predict_count += query_distances.shape[0]
            # self.nn2 += (query_distances[:, 1] <= query_distances[:, 0] * thres).sum()
            # self.nn3 += (query_distances[:, 2] <= query_distances[:, 0] * thres).sum()
            # print(self.nn2 / self.predict_count, self.nn3 / self.predict_count)
        else:
            query_single_nns = np.squeeze(query_nns, -1)
        anomaly_scores = np.mean(query_distances, axis=-1)
        if method == 'square':
            residuals = (self.detection_features[query_single_nns] - query_features) ** 2
        if method == 'abs':
            residuals = np.abs(self.detection_features[query_single_nns] - query_features)
        ...
        return anomaly_scores,residuals,query_single_nns# query_distances, query_nns
...
class NearestNeighbourScorer_gpu(object):
    def __init__(self, n_nearest_neighbours: int) -> None:
        ...
        self.nn3 = 0
        self.MAX_DIS = 99999
    def fit(self, detection_features) -> None:
    ...
    def predict(
        self, query_features,ignore_patches,nn_num,p,method='square'
    ):
        """Predicts anomaly score.
        ...
        # d = torch.cdist(query_features,self.detection_features,p=2)
        # print(self.detection_features.shape,query_features.shape) # torch.Size([85606, 1024]) torch.Size([4096, 1024]) torch.Size([4096, 85606])
        feature_distance = torch.cdist(query_features,self.detection_features,p=2)**2
        feature_distance[ignore_patches] = self.MAX_DIS
        _,query_nns =  torch.topk(feature_distance,nn_num,1,largest=False)
        # print(feature_distance[query_nns])
        if nn_num > 1:
            # p = np.zeros_like(query_distances)
            # p[query_distances[:,2] <= query_distances[:,0] * thres] = [0.5,0.3,0.2]
            # p[(query_distances[:,1] <= query_distances[:,0] * thres) * (query_distances[:,2] > query_distances[:,0] * thres)] = [0.7,0.3,0]
            # p[query_distances[:,1] > query_distances[:,0] * thres] = [1,0,0]
            indexs = np.random.choice(np.arange(len(p)),size=query_features.shape[0], p=p)
            # print(indexs)
            query_single_nns = query_nns[np.arange(query_features.shape[0]),indexs]
        ...
        if method == 'square':
            residuals = (self.detection_features[query_single_nns] - query_features) ** 2
        if method == 'abs':
            residuals = torch.abs(self.detection_features[query_single_nns] - query_features)

        if method == 'sub':
            residuals = self.detection_features[query_single_nns] - query_features
