    ...
    def get_image_data(self):
        ...
        for classname in self.classnames_to_use:
            for anomaly in ['ng','ok']:
                ...
                image_paths = sorted([*glob(os.path.join(self.source,classname,'crop','*','images',f'*{self.split.value}_{anomaly}*.png'))])
                ...
                if anomaly == 'ng' and self.split == DatasetSplit.VAL:
                    image_paths += sorted([*glob(os.path.join(self.source,classname,'crop','*','images',f'*train_{anomaly}*.png'))])
                    # mask_paths += [data_info["root_dir"] + p for p in
                    #               data_info[classname]["train"][f'{anomaly}_binary']]
                for i, image_path in enumerate(image_paths):
                    if '_pha' in image_path or '_median' in image_path: continue
                    data_tuple = [classname, anomaly, image_path]
                    if anomaly != "ok":
                        data_tuple.append(image_path.replace('.png','_pha.png'))
                    else:
