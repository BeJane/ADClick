...
from isegm.utils.serialization import get_config_repr, load_torch_checkpoint
...
class ISTrainer(object):
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            image, gt_mask, points,residual,prompt = batch_data['images'], batch_data['instances'], \
                batch_data['points'],batch_data['residual'],batch_data['prompt']
            ...
            with torch.no_grad():
                ...
                for click_indx in range(num_iters):
                    ...
                    prev_output = torch.sigmoid(eval_model(net_input, residual,prompt,points)['instances'])
                    # print(eval_model(net_input, points)['instances'].shape)
            ...
            net_input = torch.cat((image, prev_output), dim=1) if self.net.with_prev_mask else image
            output = self.net(net_input,residual,prompt, points)
            loss = 0.0
...
def get_next_points(pred, gt, points, click_indx, pred_thresh=0.49):
    ...
    points = points.clone()
    # print(torch.sum(points[0,:,-1] ==100))
...
def load_weights(model, path_to_weights):
    ...
    checkpoint = load_torch_checkpoint(path_to_weights, map_location='cpu')
    new_state_dict = checkpoint['state_dict'] if 'state_dict' in checkpoint else checkpoint
