...
class DSample:
    def __init__(self, image, encoded_masks, residual,objects=None,
    ...
    def augment(self, augmentator):
        ...
        aug_output = augmentator(image=self.image, mask=self._encoded_masks,residual=self.residual)
        self.image = aug_output['image']
        self._encoded_masks = aug_output['mask']
        self.residual = aug_output['residual']
        # if np.max(self._encoded_masks) > 0:
        #     plt.subplot(1,2,1)
        #     plt.imshow(self._encoded_masks)
        #     plt.subplot(1,2,2)
        #     plt.imshow(self.residual.mean(1).reshape(112,112))
        #     plt.show()
