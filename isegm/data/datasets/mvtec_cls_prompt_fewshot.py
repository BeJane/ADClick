...
class Mvtec_ClsPrompt_fewshot_Dataset(ISDataset):
    def __init__(self, dataset_path,category, split='train',shot=10, **kwargs):
    ...
    def get_sample(self, index) -> DSample:
        ...
        if c in ['carpet','grid','leather','tile','wood']:
            residual_path = os.path.join(sample_id.replace(bsn,''),f'{self.shot}shot_pca0.95_glo64s4pca16w4_pos0.03_l123_residual',bsn.replace('png','npy'))
        else:
            residual_path = os.path.join(sample_id.replace(bsn, ''), f'{self.shot}shot_pca0.95_glo64s4pca16w4_pos0.15_l123_residual',
                                         bsn.replace('png', 'npy'))
