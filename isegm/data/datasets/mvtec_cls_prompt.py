...
class Mvtec_ClsPrompt_Dataset(ISDataset):
    def __init__(self, dataset_path,category, split='train',cls_prompt_index=None, **kwargs):
        ...
        self.cls_prompt_index = cls_prompt_index
    ...
    def get_sample(self, index) -> DSample:
        ...
        if c in ['carpet','grid','leather','tile','wood']:
            residual_path = os.path.join(sample_id.replace(bsn,''),'pca0.95_glo64s4pca16w4_pos0.03_l123_residual',bsn.replace('png','npy'))
        else:
            residual_path = os.path.join(sample_id.replace(bsn, ''), 'pca0.95_glo64s4pca16w4_pos0.15_l123_residual',
                                         bsn.replace('png', 'npy'))
        ...
        if self.dataset_split in ['test_sup_ad', 'test_un_ad']:
            ...
            if self.cls_prompt_index is None:
                random_index = random.randint(0, len(prompt_list)-1)

                prompt = prompt_list[random_index]
            else:
                # print(self.cls_prompt_index)
                prompt = prompt_list[self.cls_prompt_index]
