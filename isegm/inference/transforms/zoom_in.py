...
class ZoomIn(BaseTransform):
    ...
    def transform(self, image_nd, residual,clicks_lists: List[List[Click]]):
        ...
        self._roi_image,self._roi_residual = get_roi_image_nd(image_nd,residual, self._object_roi, self.target_size)

        tclicks_lists = [self._transform_clicks(clicks_list)]
        return self._roi_image.to(image_nd.device), self._roi_residual.to(residual.device),tclicks_lists
    ...
    def inv_transform(self, prob_map):
        ...
        # plt.subplot(1, 2, 1)
        # plt.imshow(prob_map[0, 0].cpu())
        # plt.title(torch.min(prob_map))
        rmin, rmax, cmin, cmax = self._object_roi
        # print(self._object_roi,(rmax - rmin + 1, cmax - cmin + 1))
        ...
        if self._prev_probs is not None:
            ...
            # plt.subplot(1,2,2)
            # plt.imshow(new_prob_map[0,0].cpu())
            # plt.show()
        else:
...
def get_roi_image_nd(image_nd, residual,object_roi, target_size):
    ...
    with torch.no_grad():
        ...
        roi_image_nd = torch.nn.functional.interpolate(roi_image_nd, size=(new_height, new_width),
                                                       mode='bilinear', align_corners=True)
        residual = residual[:, :, rmin//4:rmax//4 + 1, cmin//4:cmax//4 + 1]
        residual = torch.nn.functional.interpolate(residual, size=(new_height//4, new_width//4), mode='bilinear', align_corners=True)
    return roi_image_nd,residual
