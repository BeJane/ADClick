...
class Clicker(object):
    ...
    def _get_next_click(self, pred_mask, padding=True):
        ...
        # 改成随机选点
        if is_positive:
            coords_ys, coords_xs = np.where(fn_mask_dt == fn_max_dist)  # coords is [y, x]
        else:
            coords_ys, coords_xs = np.where(fp_mask_dt == fp_max_dist)  # coords is [y, x]
        ran_index = np.random.choice(len(coords_xs))
        return Click(is_positive=is_positive, coords=(coords_ys[ran_index], coords_xs[ran_index]))
