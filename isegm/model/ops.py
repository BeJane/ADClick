...
class DistMaps(nn.Module):
    ...
    def get_coord_features(self, points, batchsize, rows, cols):
        if self.cpu_mode:
            ...
            # print(points.shape,points_order.shape)
            ...
            # print(torch.stack((coord_rows, coord_cols), dim=0).unsqueeze(0).shape,coords.shape)
            add_xy = (points * self.spatial_scale).view(points.size(0), points.size(1), 1, 1)
            # print(coords.shape,add_xy.shape) #torch.Size([192, 2, 448, 448]) torch.Size([192, 2, 1, 1])
        ...
        # plt.subplot(1,2,1)
        # plt.imshow(coords[0,0].cpu())
        # plt.subplot(1, 2, 2)
        # plt.imshow(coords[0, 1].cpu())
        # plt.show()
        return coords
