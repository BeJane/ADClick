...
from isegm.inference.evaluation import evaluate_dataset, evaluate_dataset_ad
...
def parse_args():
    ...
    parser.add_argument('--task', type=str, default='click',choices=['click','sup_ad','un_ad'])
    parser.add_argument('--cls-prompt-index', type=int,default=None,help='Specify the exact index of the CLS prompt to use, rather than choosing randomly.')
...
def main():
    ...
    for dataset_name in args.datasets.split(','):
        ...
        for checkpoint_path in checkpoints_list:
            ...
            if not args.eval_ritm and hasattr(model, 'backbone'):
            ...
            if args.task == 'click':
                dataset = utils.get_dataset(dataset_name, cfg, args.category,cls_prompt_index=args.cls_prompt_index,shot=args.few_shot)
                dataset_results,(ap,pro,pixel_auc,iou,noc80,noc85,noc90) = evaluate_dataset(dataset, predictor, pred_thr=args.thresh,
                                                   max_iou_thr=args.target_iou,
                                                   min_clicks=args.min_n_clicks,
                                                   max_clicks=args.n_clicks,
                                                   callback=vis_callback)
                if os.path.exists(f'{logs_path}/result_{args.n_clicks}.csv'):
                    f = open(f'{logs_path}/result_{args.n_clicks}.csv','a')
                else:
                    f = open(f'{logs_path}/result_{args.n_clicks}.csv', 'w')
                    f.write('dataset,ap,pro,pixel_auc,iou,noc80,noc85,noc90\n')
                f.write(f'{args.category},{ap},{pro},{pixel_auc},{iou},{noc80},{noc85},{noc90}\n')
            else:
                dataset = utils.get_dataset(dataset_name, cfg, args.category, f'test_{args.task}')
                (ap, pro, pixel_auc,image_auc) = evaluate_dataset_ad(dataset, predictor,
                                                                                  pred_thr=args.thresh,
                                                                                  max_iou_thr=args.target_iou,
                                                                                  min_clicks=args.min_n_clicks,
                                                                                  max_clicks=0,
                                                                                  callback=vis_callback)
                if os.path.exists(f'{logs_path}/result_{args.task}.csv'):
                    f = open(f'{logs_path}/result_{args.task}.csv','a')
                else:
                    f = open(f'{logs_path}/result_{args.task}.csv', 'w')
                    f.write('dataset,ap,pro,pixel_auc,image_auc\n')
                f.write(f'{args.category},{ap},{pro},{pixel_auc},{image_auc}\n')
